...
- Раз функция чистая, проще кэшировать
...
- By Name - это laziness и простота использования. `(x: => Int)`. Отличие от объявления функции в качестве параметра состоит в отсутствии скобок. то есть на вход пофигу. А выход можно объявить как `A` И типа нам всё равно. Что вернет блок кода, то вернет и обертка. Также хорош тем, что можно передавать не только функцию, но и выражение. А в Scala всё выражение.
- Laziness позволяет даже вычисление простого Int при передаче завернуть в блок, а до выполнения обернуть в try/catch в другом месте.
...
## Partial apply, curring

- Кароче, надо например `val` или `def` и вместо параметра передать `_ : Тип`. Вот это partial apply.
- А карирование проще — просто `.curried`

### Parameter Groups

- группы параметров - это по сути partial apply? (или нет? СКОРЕЕ НЕТ, потому что тогда бы не работало)
- с группами параметров легко сбацать свою управляющую структуру — ну типа блок кода, или ещё что-нибудь в скобках, сочетающееся с поведением структуры.
- implicits могут помочь сохранить код чистым, если нужно сохранить неизменными какие-то параметры (то есть без оборачивания в фунцию/объект только лишь для храниения параметров. Это как дефолтное значение аргумента, которое можно задать извне.) Для этого нужно, чтобы implicits были только в одной группе параметров - последней. Переменную implicit можно объявить после функции.
